...
@section('page_title', 'Edit Portofolio')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="p-6">
            <form action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    @if($portfolio->image)
                        <div class="mt-4">
                            <img src="{{ $portfolio->image }}" alt="{{ $portfolio->title }}" class="max-h-64 rounded-lg">
                        </div>
                    @endif
                </div>
